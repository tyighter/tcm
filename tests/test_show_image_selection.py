...
class SelectSourceImagesTests(TestCase):
    ...
    def test_tvdb_hit_skips_lower_priority_sources(self) -> None:
        show = Show.__new__(Show)
        show.card_class = SimpleNamespace(USES_UNIQUE_SOURCES=True)
        show.tvdb_skip_localized_images = False
        show.image_source_priority = ['tvdb', 'plex']
        show.library_name = 'Library'
        show.series_info = SimpleNamespace(
            full_name='Example Show',
            has_id=lambda value: value == 'tvdb_id',
        )

        show.emby_interface = None
        show.jellyfin_interface = None

        tvdb_interface = MagicMock()
        tvdb_interface.get_source_image.return_value = 'tvdb-image-url'
        tvdb_interface.is_permanently_blacklisted.return_value = False
        show.tvdb_interface = tvdb_interface

        show.tmdb_interface = None
        ...
        tvdb_interface.get_source_image.assert_called_once_with(
            show.series_info, episode_info, skip_localized_images=False
        )
        plex_interface.get_source_image.assert_not_called()
        download_image.assert_called_once_with('tvdb-image-url', source_path)

    def test_tvdb_miss_falls_back_to_tmdb(self) -> None:
        show = Show.__new__(Show)
        show.card_class = SimpleNamespace(USES_UNIQUE_SOURCES=True)
        show.tvdb_skip_localized_images = False
        show.tmdb_skip_localized_images = False
        show.image_source_priority = ['tvdb', 'tmdb']
        show.library_name = 'Library'
        show.series_info = SimpleNamespace(
            full_name='Example Show',
            has_id=lambda value: value == 'tvdb_id',
        ...
        tvdb_interface = MagicMock()
        tvdb_interface.get_source_image.return_value = None
        tvdb_interface.is_permanently_blacklisted.return_value = False
        show.tvdb_interface = tvdb_interface

        tmdb_interface = MagicMock()
        tmdb_interface.get_source_image.return_value = 'tmdb-image-url'
        tmdb_interface.is_permanently_blacklisted.return_value = False
        show.tmdb_interface = tmdb_interface

        show.plex_interface = None
        ...
        download_image.assert_called_once_with('tmdb-image-url', source_path)
